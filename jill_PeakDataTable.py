...
def Create_Feature_Array(data, enhancerSignals, tssSignals, optionalSignals, geneDict):
    ...
    for pair in data:
        ...
        features.append([pair[0]+"-"+pair[1]] +enhancerSignals[pair[0]]+tssFeatures+ \
            optionalFeatures)
...
##
trainFeatFile = open(sys.argv[i+4], "w")
for i in range(len(trainFeat)):
    trainFeat[i].append(trainLab[i])
    trainFeatFile.write('{0}\n'.format("\t".join(str(j) for j in trainFeat[i])))
trainFeatFile.close()
